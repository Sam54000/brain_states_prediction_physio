...
def detect_high_frequency_artifacts(
    signal: np.ndarray,
    fs: float,
    window_size: float = 1.0,
    step_size: float = 0.25,
    highpass_cutoff: float = 10.0,
    amplitude_threshold_multiplier: float = 1.0,
    buffer: float = 0.5
) -> np.ndarray:
    """
    Detects high frequency, high amplitude transient artifacts in physiological signals.
    ...
    3. Marking segments where the amplitude exceeds a threshold (default: 2 × standard deviation)
    
    Args:
        signal (np.ndarray): 1D numpy array of the physiological signal
        fs (float): Sampling frequency of the signal in Hz
        window_size (float, optional): Size of the sliding window in seconds. Defaults to 1.0.
        step_size (float, optional): Step size for the sliding window in seconds. Defaults to 0.25.
        ...
        buffer (float, optional): Buffer in seconds to extend the mask. Defaults to 0.5.
    Returns:
    ...
    high_freq_signal = filter_signal(
        signal=signal,
        fs=fs,
        lowcut=highpass_cutoff,
        highcut=None
    )
    
    
    analytic_signal = scipy.signal.hilbert(high_freq_signal)
    envelope = np.abs(analytic_signal)
    envelope -= np.mean(envelope)
    
    threshold = np.std(envelope) * amplitude_threshold_multiplier
    ...
    for start in range(0, signal_length - window_samples + 1, step_samples):
        ...
        if start-buffer_samples < 0:
            mask[:end+buffer_samples] = False
        elif end+buffer_samples > len(mask):
            mask[start-buffer_samples:] = False
        else:
            mask[start-buffer_samples:end+buffer_samples] = False
...
def detect_high_gradient(signal: np.ndarray, fs: float, threshold: float = 10, buffer: float = 0.5) -> np.ndarray:
    """
...
def detect_flat_signal(
    ...
    derivative = np.abs(np.diff(smoothed, prepend=smoothed[0]))
...
def test_artifact_detectors(save_path: Optional[os.PathLike] = None):
    ...
    hf_rsp_mask = detect_high_frequency_artifacts(
        signal=rsp_signal,
        fs=fs,
        window_size=1.0,
        step_size=0.25,
    ...
    flat_rsp_mask = detect_flat_signal(
        signal=rsp_signal,
        fs=fs,
        flat_duration_threshold=1.0
    )
    
    combined_rsp_mask = hf_rsp_mask & flat_rsp_mask
    
    
    hf_ecg_mask = detect_high_frequency_artifacts(
        signal=ecg_signal,
        fs=fs,
        window_size=0.5,
        step_size=0.1,
    ...
    axes[1].plot(t, np.logical_not(hf_rsp_mask) * 1.0, 'r-', label='High-freq artifacts')
    axes[1].plot(t, np.logical_not(flat_rsp_mask) * 0.8, 'g-', label='Flat segments')
    axes[1].plot(t, np.logical_not(combined_rsp_mask) * 0.6, 'b-', label='Combined mask')
    ...
    axes[3].plot(t, np.logical_not(hf_ecg_mask) * 1.0, 'r-', label='High-freq artifacts')
    axes[3].plot(t, np.logical_not(flat_ecg_mask) * 0.8, 'g-', label='Flat segments')
    axes[3].plot(t, np.logical_not(combined_ecg_mask) * 0.6, 'b-', label='Combined mask')
...
if __name__ == "__main__":
    ...
    video_path = '/Users/samuel/Desktop/processed/18818841.mp4'
