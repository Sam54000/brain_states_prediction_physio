...
if __name__ == "__main__":
    ...
    for subject in subjects:
        ...
        for session in sessions:
            for task in tasks:
                print(f"Processing {subject} {session} {task}")
                file_parts = [
                    f"sub-{subject}",
                    f"ses-{session}",
                    f"task-{task}",
                ...
                with PdfPages(pdf_filename) as pdf:
                    ...
                    if biopac and not eyetracking:
                        report["message"].append("Biopac found but no Eyetracking")
                    if not biopac and eyetracking:
                        report["message"].append("Eyetracking found but no Biopac")
                    if ecg_error:
                        report["message"][-1] += "(Neurokit2 error with ECG)"
                pdf.close()
